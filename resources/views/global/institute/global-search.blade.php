      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              <div class="p-6 bg-white border-b border-gray-200">
                ...
                <!-- searched Institute -->
                @if(isset($search_institutes) &&  $search_institutes != null)
                <div class="mb-20">
                  <h2 class="font-bold text-2xl mb-4">Search Result:</h2> 
                  <hr>
                  <div class="grid grid-cols-2 gap-4 mt-5">
                    @foreach ($search_institutes as $institute)
                      <div>
                    ...
                    <div class="col-span-2">
                      {{ $search_institutes->appends(['latest_institute_page' => $latest_institutes->currentPage() ])->appends(['popular_institute_page' => $popular_institutes->currentPage() ])->links() }}
                    </div>
                  </div>

                  @if (count($search_institutes) <= 0)
                ...
                <!-- Latest Institute -->
                <div>
                  <h2 class="font-bold text-2xl mb-4">Latest institutes:</h2>
                  <hr>
                  <div class="grid grid-cols-2 gap-4 mt-5 mb-20">
  
                    @if(isset($latest_institutes) &&  $latest_institutes != null)
                      @foreach ($latest_institutes as $institute)
                        <div>
                          <a href="{{ route('institute.view-single', $institute->id) }}">
                            <div class="max-w-7xl mx-auto">
                              <div class="relative group">
                      ...
                      <div class="col-span-2">
                        {{ $latest_institutes->appends(['popular_institute_page' => $popular_institutes->currentPage() ])->links() }}
                      </div>
                ...
                <!-- Popular Institute -->
                <div>
                  <h2 class="font-bold text-2xl mb-4">Popular institutes:</h2>
                  <hr>
                  <div class="grid grid-cols-2 gap-4 mt-5 mb-20">
  
                    @if(isset($popular_institutes) &&  $popular_institutes != null)
                    @foreach ($popular_institutes as $institute)
                      <div>
                        <a href="{{ route('institute.view-single', $institute->id) }}">
                          <div class="max-w-7xl mx-auto">
                            <div class="relative group">
                    ...
                    @endforeach
                    <div class="col-span-2">
                      {{ $popular_institutes->appends(['latest_institute_page' => $latest_institutes->currentPage() ])->links() }}
                    </div>
                    @else
                      <h3 class="text-black font-bold">
                        No institute Found
